...
int destroy() {
	...
	if (remove(FILE_NAME) == 0) {
		...
		perror("Error al inicializar servicio");
		return -1;
	}
}
...
int set_value(int key, char *value1, int N_value2, double *V_value2, struct Coord value3) {	
	...
	fprintf(file, "Valor 1: '%s'\n", value1);
	fprintf(file, "Valor 2: %d\n", N_value2);
...
int get_value(int key, char *value1, int *N_value2, double *V_value2, struct Coord *value3) {
	...
	/*value1*/
	if (!fgets(line, sizeof(line), file) || sscanf(line, "Valor 1: '%[^\']", value1) != 1) {
		fclose(file);
		return -1;
	}

	/*^N_value2*/
	if (!fgets(line, sizeof(line), file) || sscanf(line, "Valor 2: %d", N_value2) != 1) {
		fclose(file);
		return -1;
	}
...
int delete_key(int key) {
	...
	while (fgets(line, sizeof(line), file)) {
		...
		if (sscanf(line, "Clave: %d", &existing_key) == 1 && existing_key == key) {
			...
			found = 1;
			/*Ignorar value1, N_value2, V_value3, value3 y el espacio de separación "\n"*/
